...
class FIFOQueueBB(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A = sorted(self.A, key=lambda Node: Node.path_cost)
...
class FIFOQueueBBEstimate(Queue):
    """A First-In-First-Out Queue."""
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A = sorted(self.A, key=lambda Node: Node.path_cost + self.problem.h(Node))
